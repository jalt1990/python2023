    ...
    while pin_ok:
        ...
        if scelta == 'exit':
         ...
         # scelta 2 (visualizza saldo)    
            ...
            if utente_corrente == 1:
                beneficiario = input("Inserisci il beneficiario: ")
                importo = int(input('Inserisci importo del bonifico: '))
                if u1_conto > importo:
                    u1_conto -= importo
                    print(f'Hai appena inviato a {beneficiario} un bonifico di {importo} €.')
                    print(f'Il tuo nuovo saldo è di {u1_conto}')
                else:
                    print("Non ci sono abbastanza fondi per effettuare l'operazione")
            else:
                beneficiario = input("Inserisci il beneficiario: ")
                importo = int(input('Inserisci importo del bonifico: '))
                if u2_conto > importo:
                    u2_conto -= importo
                    print(f'Hai appena inviato a {beneficiario} un bonifico di {importo} €.')
                    print(f'Il tuo nuovo saldo è di {u2_conto}')
                else:
                    print("Non ci sono abbastanza fondi per effettuare l'operazione")
            ...
            if utente_corrente == 1:
                print("Scegli l'operatore tra i seguenti:\n-1.TIM\n-2.VODAFONE\n-3.WIND")
                operatore = ['TIM', 'VODAFONE', 'WIND'][int(input("scelta: "))-1]

                print("Scegli il taglio della ricarica:\n-1. 10€\n-2. 15€\n-3. 30€\n-4.50€\n-5. 100€")
                importo = [10,15,30,50,100][int(input("scelta: "))-1]

                numero_tel = input('Inserisci il numero di telefono:/n')
                if u1_conto > importo:
                    u1_conto -= importo
                    print(f'Hai appena ricaricato il numero {numero_tel}.')
                    print(f'Il tuo nuovo saldo è di {u1_conto}')
            else:
                beneficiario = input("Inserisci il beneficiario: ")
                importo = int(input('Inserisci importo del bonifico: '))
